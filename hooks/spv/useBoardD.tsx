...
const deleteABRequest = async (id: string, index: number) => {
  const res = await api.delete(`/spv/${id}/board-members/${index}`);
  return res.data.data;
};
...
const useABApi = () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async ({ id, indexNo }: { id: string; indexNo: number }) => {
      const res = await deleteABRequest(id, indexNo);
  ...
  return {
    ...
    responseData:
      createMutation.data || updateMutation.data || deleteMutation.data,
  };
};
