...
const useABApi = () => {
  ...
  const createAB = async (data: BoardMemberPayload) => {
    ...
    try {
      ...
      setStatus('error');
      const message = err.response?.data?.message || 'Failed to create board member';
      setError(message);
      toast.error(message);
      throw err;
    }
  ...
  const updateAB = async (id: string, data: Partial<BoardMemberPayload>) => {
    setStatus('loading');
    setError(null);
    setResponseData(null);
    try {
      ...
      setStatus('error');
      const message = err.response?.data?.message || 'Failed to update board member';
      setError(message);
      toast.error(message);
      throw err;
    }
  ...
  const deleteAB = async (id: string) => {
    ...
    try {
      ...
      setStatus('error');
      const message = err.response?.data?.message || 'Failed to delete board member';
      setError(message);
      toast.error(message);
      throw err;
    }
