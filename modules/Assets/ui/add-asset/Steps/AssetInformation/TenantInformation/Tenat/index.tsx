...
const TenantManagement = () => {
  ...
  const [createTenant, setCreateTenant] = useState<any>(null);
  const [updateTenant, setUpdateTenant] = useState<any>(null);
  const [deleteTenant, setDeleteTenant] = useState<any>(null);
  // const { getAsset } = useAssetApi();
  const [getAsset, setGetAsset] = useState<any>(null);
    ...
    trigger(`tenants.${index}`).then(async (isValid: boolean) => {
      if (isValid) {
        ...
        if (isEdit) {
          ...
          await createTenant({ ...data, assetId: assetId ?? "" }).then((res: any) => {
            append({ ...data, _id: res._id });
          });
        }
        if (assetId) {
          await getAsset(assetId).then((res: any) => {
            if (res) {
              reset({
                ...res,
