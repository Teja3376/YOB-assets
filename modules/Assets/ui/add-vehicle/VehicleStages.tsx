...
export default function VehicleStages({
    ...
    switch (stepId) {
      case "vehicle-identification":
        return (
          ...
          isFilled(data.model) &&
          isFilled(data.trim) &&
          isFilled(data.bodyType) &&
          isFilled(data.year) &&
          isFilled(data.vin) &&
          isFilled(data.odometer) &&
          isFilled(data.exteriorColor) &&
          isFilled(data.interiorColor) &&
          isFilled(data.about)
        );

      case "engine-specs":
        return (
          isFilled(data.engineDisplacement) &&
          isFilled(data.cylinders) &&
          isFilled(data.horsePower)

          // isFilled(data.totalNumberOfSfts) &&
          // isFilled(data.pricePerSft)
        );

      case "valuation-investment":
        return (
          isFilled(data.investmentStats?.startingValue) &&
          isFilled(data.investmentStats?.targetFinalValue) &&
          isFilled(data.investmentStats?.appreciationExpectedYearly) &&
          isFilled(data.investmentStats?.minReturnToInvestors) &&
  ...
  const { stepIndex, progress, totalSteps, completedSteps } = useMemo(() => {
    const index = VEHICLE_STEPS_TABS.findIndex(
      (step: any) => step.id === currentStep,
    );
    const totalSteps = VEHICLE_STEPS_TABS.length;

    const completedSteps = VEHICLE_STEPS_TABS.filter((step: any) =>
      hasStepData(step.id),
    ).length;
  ...
  const assetData = useMemo(() => {
    ...
    return {
      ...
      assetName: `${data.brand} ${data.model}` || "Vehicle",
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-5 pb-2 pt-4">
            <div className="mt-2">
              <div className="mb-2 flex items-center justify-between px-2">
                <img
                  src={"/svg/assets/car.svg"}
                  alt="property"
                  className="w-18 h-18 object-contain"
                />
                <ArrowRightIcon className="h-4 w-4" />
                <img
                  src={"/svg/assets/file.svg"}
                  alt="tokens"
            ...
            <div className="mt-6 space-y-0">
              <FormField
                label="Asset ID"
                value={maskId(assetData.assetId, "VEH")}
...
function FormField({
  ...
  return (
    <div
      className={cn(
        "flex items-start justify-between gap-3 border-b border-gray-100 pb-3 last:border-b-0 last:pb-0 mt-3",
        className,
      )}
    >
