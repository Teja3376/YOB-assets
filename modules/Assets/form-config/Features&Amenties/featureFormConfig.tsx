...
export const featureFormConfig = (index: number): FormFieldConfig[] => {
  ...
  const watchFeatureName = watch(`features.${index}.name`);

  const options =
    watchFeatureName &&
    !baseOptions.some((opt) => opt.value === watchFeatureName)
      ? [{ label: watchFeatureName, value: watchFeatureName }, ...baseOptions]
      : baseOptions;

  const onFeatureNameChange = (value: string) => {
    const preset = FEATURE_DEFAULT_DESCRIPTIONS[value];
    if (preset !== undefined) {
      setValue(descriptionPath, preset, {
        shouldDirty: true,
        shouldValidate: true,
      });
    }
  ...
  return [
    {
      ...
      rules: {
        ...
        maxLength: {
          value: 50,
          message: "Feature Name must be at most 50 characters",
        },
      },
      allowCreate: true,
